...
class NetworkWordCountSpec extends FlatSpec with SparkStreamingSpec with GivenWhenThen with Matchers with Eventually {
  ...
  "Sample set" should "be counted" in {
    ...
    NetworkWordCount.count(ssc.queueStream(lines), Set()) { (wordsCount: RDD[WordCount], time: Time) =>
