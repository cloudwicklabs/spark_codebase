...
import com.cloudwick.spark.examples.WordCount
import com.cloudwick.spark.streaming.local.NetworkWordCountWindowed._
 ...
 *    `nc -lk 9999`
 * and then run this example (NOTE: the master has to be local because we are binding nc to the localhost)
 *    `spark-submit --class com.cloudwick.spark.streaming.local.NetworkWordCountWindowed --master local[*] <path_to_jar> localhost 9999`
 */
object NetworkWordCountWindowedRunner extends App with Logging {
  ...
  log.info("Connecting to host: " + hostname + " port: " + port)
  ...
  NetworkWordCountWindowed.count(lines, windowDuration, slideDuration, stopWords) { (wordsCount: RDD[WordCount], time: Time) =>
